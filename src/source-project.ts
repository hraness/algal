...
import {
  ...
  SourceError, type SourceCompilation, type SourceSpan,
} from "./source";
...
type Location = { file: string; span: SourceSpan };
const fail = (message: string, location: Location): never => { throw new SourceError(`${location.file}: ${message}`, location.span); };
const sameFile = (left: Stats, right: Stats): boolean => left.dev === right.dev && left.ino === right.ino;
function importedKey(importer: string, path: string, span: SourceSpan): string {
  try { return resolveSourceImport(importer, path, span); }
  catch (error) {
    if (error instanceof SourceError) throw new SourceError(`${importer}: ${error.diagnostic.message}`, error.diagnostic.span);
    throw error;
  }
}
...
async function boundedSource(root: string, key: string, remainingBytes: number, location: Location): Promise<{ source: string; bytes: number }> {
  try {
    ...
    try {
      ...
      if (before.size > limit) fail(before.size > SOURCE_BOUNDS.maxSourceBytes ? `source exceeds ${SOURCE_BOUNDS.maxSourceBytes} UTF-8 bytes: ${key}` : `source project exceeds ${SOURCE_PROJECT_BOUNDS.maxTotalBytes} bytes`, location);
      ...
      catch { return fail(`source is not valid UTF-8: ${key}`, location); }
...
export async function loadSourceProject(entryPath: string, options: SourceProjectOptions = {}): Promise<SourceProject> {
  const location: Location = { file: entryPath, span: start };
  ...
  const entry = importedKey("entry.algal", `./${candidate}`, start);
  const sources: Record<string, string> = Object.create(null);
  const heights = new Map<string, number>();
  const active = new Set<string>();
  let totalBytes = 0;
  const visit = async (key: string, depth: number, origin: Location): Promise<number> => {
    ...
    const loaded = await boundedSource(root, key, SOURCE_PROJECT_BOUNDS.maxTotalBytes - totalBytes, origin);
    ...
    try { imports = sourceImports(loaded.source); }
      ...
      if (error instanceof SourceError) throw new SourceError(`${key}: ${error.diagnostic.message}`, error.diagnostic.span);
      throw error;
    }
    if (imports.length > SOURCE_PROJECT_BOUNDS.maxImports) fail(`source file exceeds ${SOURCE_PROJECT_BOUNDS.maxImports} imports: ${key}`, { file: key, span: imports[SOURCE_PROJECT_BOUNDS.maxImports]!.span });
    let height = 0;
    for (const item of imports) {
      const imported = importedKey(key, item.path, item.span);
      height = Math.max(height, 1 + await visit(imported, depth + 1, { file: key, span: item.span }));
    }
    active.delete(key); heights.set(key, height);
    return height;
  };
  await visit(entry, 0, { file: entry, span: start });
