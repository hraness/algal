...
import { mintInterappMessage } from "./application-message-contract";
...
export interface ApplicationAdmission {
  ...
  admitCommit(context: {
    ...
    pending: ApplicationPending[]; store: Store;
  }): Promise<void>;
  ...
  admitDispatch?(context: {
    ...
    previousDispatch: ApplicationDispatch | null; store: Store;
  }): Promise<unknown>;
}
...
/** One lifecycle algorithm for filesystem and host-injected atomic storage.
 * The storage port owns namespace custody/durability; admission owns authority. */
export class ApplicationCore {
  readonly store: Store;
  constructor(private readonly storage: ApplicationStorage, private readonly admission: ApplicationAdmission, private readonly options: ApplicationOptions = {}) {
  ...
  async history(application: unknown): Promise<ApplicationSnapshot[]> {
    ...
    // A read never reserves a namespace. Storage validates its own custody boundary.
    const raw = await this.storage.readHead(name);
  ...
  private async dispatchRecord(application: string, ref: Digest, work: WorkIntent): Promise<ApplicationDispatch | null> {
    const raw = await this.storage.readDispatch(application, ref);
  ...
  async commit(input: unknown): Promise<ApplicationSnapshot> {
    ...
    return this.storage.custody(command.application, true, async () => {
      ...
      const operationCount = await this.storage.operationCount(command.application);
      if (!prepared && operationCount >= APPLICATION_LIMITS.states) throw new Error("Application operation bound exhausted");
      const head = json({contract: "algal.application-head.v1", application: command.application, state: next.digest});
      await this.storage.publication(command.application, [json(operation), head], async () => {
  ...
  private async execute(current: ApplicationSnapshot, snapshot: ApplicationSnapshot, work: WorkIntent, record: ApplicationDispatch, dispatcher: ApplicationDispatcher, reconciliation: boolean): Promise<ApplicationDispatch> {
    ...
    if (!reconciliation) {
      await this.storage.publication(record.application, [json(record)], () => this.storage.writeDispatch(record.application, record.intent, json(record), true));
    ...
    await this.storage.publication(record.application, [json(updated)], () => this.storage.writeDispatch(record.application, record.intent, json(updated), false));
  ...
  async dispatchPending(application: unknown, dispatcher: ApplicationDispatcher, max: unknown = 32): Promise<ApplicationDispatchAttempt[]> {
    ...
    return this.storage.custody(name, false, async () => {
