...
export type ApplicationTransition = {
  ...
  operation: Digest;
  request: Digest;
  kind: "create" | "memory" | "investigate" | "activate" | "migrate";
...
export function parseApplicationTransition(input: unknown): ApplicationTransition {
  ...
  if (v.kind !== "create" && v.kind !== "memory" && v.kind !== "investigate" && v.kind !== "activate" && v.kind !== "migrate") throw new Error("Invalid application transition kind");
