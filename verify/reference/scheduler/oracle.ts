/** Independent bounded-fixture scheduler oracle. No production imports. */
...
export type Cell = Base & (
  {kind:'input'} | {kind:'const';values:Values} |
...
type Frame = {
  ...
  iteration:number;carried:Values;aggregate:Values;lastChild:Values;
};
export type State = {
  frames:Frame[];budget:Budget;cells:Record<string,CellRecord>;effects:Effect[];
  ...
  mutation:'none'|'reverse-edges'|'reset-budget'|'reuse-path'|'drop-carry'|'suspend-failure'|'widen-bytes'|'refund-retry';
};
...
function admitProgram(raw:unknown,depth=0,bound={programs:0,cells:0}):asserts raw is Program{
 ...
 for(const c of raw.cells){
  ...
  const extra=kind==='input'?[]:kind==='const'?['values']:kind==='fn'?['op']:kind==='agent'?['prompt','attempts','budget']:kind==='organism'?['child']:kind==='repeat'?['child','max','carry','until']:['child','max','over'];
  ...
  insist(['input','const','fn','agent','organism','repeat','each'].includes(String(kind)),'supported kind');
...
export function start(program:unknown,args:unknown,budget:unknown,mutation:State['mutation']='none'):State{
 insist(['none','reverse-edges','reset-budget','reuse-path','drop-carry','suspend-failure','widen-bytes','refund-retry'].includes(mutation),'mutation');
 admitProgram(program);fields(budget,['maxSteps','maxAgentCalls','maxWork','maxDepth','maxContextBytes','maxOutputBytes'],['maxSteps','maxAgentCalls','maxWork','maxDepth','maxContextBytes','maxOutputBytes'],'root budget');
 ...
 return structuredClone({frames:[frame(program,args as Record<string,Values>,'',0)],budget:budget as Budget,cells:{},effects:[],work:{steps:0,agentCalls:0,units:0},outcome:'running',trace:[],guardCounts:{},mutation});
}
...
export function step(previous:State,response?:Response):State{
 ...
 switch(f.phase){
  ...
  case 'await':{
   ...
   f.lastResponse=structuredClone(response);f.effectOrdinal=s.effects.length;s.effects.push({ordinal:s.effects.length,path:at,request:f.request!,response:structuredClone(response)});f.phase='bind';action='return-effect';break;
  }
  case 'bind':{
  ...
  case 'push':{
   insist(c.kind==='organism'||c.kind==='repeat'||c.kind==='each','wrapper kind');
   const suffix=c.kind==='organism'?'':`/${c.kind==='repeat'?'r':'i'}${s.mutation==='reuse-path'?0:f.iteration}`;
   const childPath=at+suffix,depth=f.depth+1;
   ...
   const supplied={...f.inputs,...(s.mutation==='drop-carry'?{}:f.carried)};
   if(c.kind==='each'){
   ...
   const args:Record<string,Values>={};for(const [alias,t]of Object.entries(c.child.interface.inputs)){const v=supplied[alias];if(v!==undefined)(args[t.cell]??={})[t.port]=v;}
   f.phase='child';s.frames.push(frame(c.child,args,childPath,depth));
  ...
  case 'terminal':{
   ...
   insist(wrapper.kind==='organism'||wrapper.kind==='repeat'||wrapper.kind==='each','return wrapper');
  ...
  case 'return':{
   insist(c.kind==='organism'||c.kind==='repeat'||c.kind==='each','return continuation');
   f.iteration++;
...
export function execute(program:Program,args:Record<string,Values>,budget:Budget,tape:Tape=[],mutation:State['mutation']='none'):State{
 let state=start(program,args,budget,mutation),cursor=0;
