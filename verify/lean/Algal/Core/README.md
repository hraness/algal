...
| `RationalSelector` | Constructive nearest-even selection for nonnegative rational magnitudes over finite binary64 values, with explicit sign reflection and an explicit zero sign. Every selected value is nearest among finite candidates; an odd selection is strictly nearest, so equal-distance competitors force even selection. Midpoint ties, both zero signs and the 63-comparison bracket search are covered. Inputs at or beyond the overflow threshold return the finite maximum; IEEE overflow admission, decimal conversion and production refinement remain separate. |
...
`RationalSelector` chooses one of the bracketed adjacent values by exact midpoint comparison and takes the even neighbor at a tie. The selected value is nearest among all finite sign-bit-zero candidates, and signed selection reflects that result under an explicit sign, so both zero signs come from the argument rather than from the rational value zero. Concrete witnesses fix the underflow, subnormal-to-normal, unit and maximum ties. A rejected witness shows that the overflow-boundary input still selects the finite maximum while lying outside the maximum cell, so this selector does not decide IEEE overflow. The comparison bound counts bracket predicate probes only; midpoint and parity comparisons, rational arithmetic, allocation and time remain unbounded.
