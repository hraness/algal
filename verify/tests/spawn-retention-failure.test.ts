...
for (const scenario of ["first-worker-failure", "producer-source-drift", "copied-reader-failure"] as const) test(`registered spawn retains only diagnostic evidence after ${scenario}`, async () => {
  ...
  try {
    ...
    expect(frame).toEqual({ scenario, primaryPreserved: true, publishedSuccess: false, diagnosticManifests: 1, admittedManifests: 0,
      incompleteCopies: scenario === "copied-reader-failure" ? 1 : 0, commands: scenario === "first-worker-failure" ? 1 : 88,
      readerCalls: scenario === "first-worker-failure" ? 0 : scenario === "producer-source-drift" ? 1 : 2,
      copiedReaderCalls: scenario === "copied-reader-failure" ? 1 : 0, rawFiles: scenario === "first-worker-failure" ? 4 : scenario === "producer-source-drift" ? 406 : 405 });
