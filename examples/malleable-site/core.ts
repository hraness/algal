/** Portable experimental host. Owner edits are explicit compatibility admissions,
 * not accepted optimization evaluations. No dispatcher/effect authority. */
import { ApplicationCore, type ApplicationAdmission, type ApplicationSnapshot } from "../../src/application-core";
...
export class MarketingCore {
  ...
  constructor(storage: ApplicationStorage, service?: ApplicationCore) { this.service = service ?? new ApplicationCore(storage, ownerEditAdmission()); }
...
export async function exportEvidence(host: MarketingCore, additionalRoots: readonly Digest[] = []): Promise<SurfaceEvidence> {
  ...
  if (additionalRoots.length > 128) throw new Error("Surface additional root bound exceeded");
  const states = history.map(row => row.digest), records: ExportRecord[] = [], seen = new Set<Digest>(), pending = [...states, ...additionalRoots.map(applicationRef)];
  let bytes = 0;
  while (pending.length) {
    ...
    bytes += new TextEncoder().encode(canonicalize(json(record))).length;
...
/** Parse each independently bounded CAS record without applying the much
 * smaller single-record limit to the complete portable archive. */
export function parseEvidence(input: unknown, expectedHead: Digest): SurfaceEvidence {
  const serialized = JSON.stringify(input);
  if (typeof serialized !== "string" || new TextEncoder().encode(serialized).length > EXPORT_MAX_BYTES) throw new Error("Surface evidence byte bound exceeded");
  if (!input || typeof input !== "object" || Array.isArray(input) || ![Object.prototype, null].includes(Object.getPrototypeOf(input))) throw new Error("Invalid surface evidence");
  ...
  const records = v.records.map((raw): ExportRecord => {
    ...
    seen.add(reference);
    if (row.kind !== "value" && row.kind !== "manifest" && row.kind !== "receipt") throw new Error("Unknown evidence record kind");
    if (row.kind === "manifest") parseOrganismManifest(row.value);
    if (row.kind === "receipt") parseRunReceipt(row.value);
    return { kind: row.kind, reference, value: applicationJson(row.value) };
  });
  return { contract: "algal.marketing-evidence.v1", claim: "content-integrity-and-pure-replay", head: expectedHead, states, records };
}
...
export async function verifyEvidence(input: unknown, expectedHead: Digest): Promise<{ ok: true; head: Digest; states: number; receipts: number; claim: string }> {
  const v = parseEvidence(input, expectedHead), states = v.states;
  {
    const host = new MarketingCore(new MemoryApplicationStorage()), store = host.service.store, seen = new Set<Digest>();
    let receipts = 0;
...
/** Verify in a separate volatile host before writing to a fresh destination.
 * The destination is a new local copy, never permission to replace an owner. */
export async function importEvidence(input: unknown, expectedHead: Digest, storage: ApplicationStorage): Promise<MarketingCore> {
  const checked = parseEvidence(input, expectedHead);
  await verifyEvidence(checked, expectedHead);
  return storage.custody("marketing-evidence-import", false, async () => {
    if (await storage.readHead(APPLICATION) !== undefined || await storage.operationCount(APPLICATION) !== 0) throw new Error("Evidence import requires a fresh application destination");
    const evidence = checked, host = new MarketingCore(storage), store = storage.store;
    for (const row of evidence.records) {
      if (row.kind === "value") await store.putValue(row.value);
      else if (row.kind === "manifest") await store.putManifest(parseOrganismManifest(row.value));
      else await store.putReceipt(row.value);
    }
    for (const reference of evidence.states) {
      const state = await getApplicationRecord(store, reference, parseApplicationState), transition = await getApplicationRecord(store, state.transition, parseApplicationTransition);
      const snapshot = await host.service.commit({ application: transition.application, operation: transition.operation, kind: transition.kind, expectedHead: transition.previous, revision: transition.revision, memory: transition.memory, intents: [], evidence: transition.evidence, causedBy: transition.causedBy });
      if (snapshot.digest !== reference) throw new Error("Imported evidence replay mismatch");
    }
    return host;
  });
}
