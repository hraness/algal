/** Owner-local pure application host. There is deliberately no dispatcher. */
import { mkdtemp, rm } from "node:fs/promises";
import { tmpdir } from "node:os";
import { join } from "node:path";
import { ApplicationService, type ApplicationSnapshot, type ApplicationAdmission } from "../../src/application";
...
async function replay(store: Store, ref: Digest, manifest: OrganismManifest, input: Record<string, JsonValue>, output: unknown): Promise<void> {
  ...
  const args = Object.fromEntries(Object.entries(input).map(([key, value]) => [key, { value }]));
  if (!same(receipt.args, args) || receipt.outcome !== "complete" || !same(receipt.cells.result?.outputs?.out, output) || !(await verifyReceipt(receipt, manifestToJson(manifest), store)).ok) throw new Error("Triage receipt binding/replay failed");
}
...
export class TriageHost {
  ...
  constructor(readonly directory: string, application = "local-triage") {
    ...
    this.service = new ApplicationService(directory, admission);
    const memoryAdmission: MemoryAdmissionHost = {
      ...
      decodeObservation: async ({ observation, raw, procedure }) => {
        ...
        await replay(this.service.store, observation.receipt, claimsManifest(version), { tasks }, { claims });
        return claims;
      },
    };
  ...
  private async fixed(version: 1 | 2, mode: "references" | "install" | "verify" = "references") {
  ...
  async current(): Promise<ApplicationSnapshot> { const current = await this.service.inspect(this.application); if (!current) throw new Error("Initialize this local triage application first"); return current; }
  ...
  private async at(head: Digest): Promise<ApplicationSnapshot> { const snapshot = (await this.service.history(this.application)).find(s => s.digest === head); if (!snapshot) throw new Error("Unknown retained triage head"); return snapshot; }
  ...
  async capture(session: Session = DEFAULT_SESSION): Promise<Capture> { return this.captureAt(await this.current(), session); }
  private sessionPath(sessionId: string): string { return join(this.directory, "triage-sessions", this.application, id(sessionId), "session.json"); }
  async loadSession(sessionId: string): Promise<SessionLoad> {
    const raw = await hostRead(this.sessionPath(sessionId), 4096);
    ...
    if (v.contract !== "algal.triage-session-record.v1" || v.application !== this.application || (v.schemaVersion !== 1 && v.schemaVersion !== 2)) throw new Error("Invalid retained session; file preserved for reconciliation");
    ...
    if (origin.state.revision !== record.capturedRevision || (await this.definition(origin.state.revision)).schemaVersion !== record.schemaVersion) throw new Error("Retained session binding changed; file preserved");
  ...
  async saveSession(sessionId: string, input: { expectedSession: Digest | null; capturedHead: Digest; session: Session }): Promise<SessionLoad> {
    ...
    const path = this.sessionPath(sessionId), record: SessionRecord = { contract: "algal.triage-session-record.v1", application: this.application, capturedHead, capturedRevision: current.revision, schemaVersion: current.definition.schemaVersion, session };
    await hostLease(join(this.directory, "triage-sessions", this.application, id(sessionId)), `triage-session-${id(sessionId)}`, async () => {
      const retained = await hostRead(path, 4096);
      if ((retained === undefined ? null : hash(retained)) !== expectedSession) throw new Error("Newer session draft exists; explicit merge required");
      await hostWrite(path, applicationJson(record), 4096, false);
    });
    return this.loadSession(sessionId);
  }
  async initialize(input: { schemaVersion?: 1 | 2; config?: Config; tasks?: Task[] } = {}, source: Digest | null = null): Promise<Capture> {
    const definition = makeRevision(input.config ?? DEFAULT_CONFIG, input.schemaVersion ?? 1), tasks = parseTasks(input.tasks ?? []), revision = await this.revisionRecord(definition, null), memory = await this.facts(tasks, definition.schemaVersion, null);
  ...
  async command(input: Command): Promise<Capture> {
    const command = parseCommand(input), prior = await this.at(command.expectedHead), definition = await this.definition(prior.state.revision), before = await this.tasks(prior), after = updateTasks(before, command.action, definition);
  ...
  async proposeEvaluate(input: Proposal): Promise<{ reference: Digest; evaluation: Evaluation; capture: Capture; preview: { definition: Revision; view: Capture["view"]; why: Capture["why"] } }> {
    const proposal = parseProposal(input), current = await this.exact(proposal.expectedHead), { evaluation, definition } = await this.evaluate(proposal), reference = await this.put(evaluation), capture = await this.captureAt(current, DEFAULT_SESSION);
  ...
  private async evaluation(ref: Digest): Promise<Evaluation> {
    ...
    for (const [i, session] of sessions.entries()) await replay(this.service.store, receipts[i]!, viewManifest(definition), { tasks, session }, evaluateView(definition, tasks, session));
    ...
    if (!same(raw, expected) || !expected.accepted) throw new Error("Rejected or forged evaluation"); return expected;
  }
  ...
  private async admit({ command, current, revision, pending }: Parameters<ApplicationAdmission["admitCommit"]>[0]): Promise<void> {
    ...
    if (command.kind === "memory") {
      ...
      if (raw && typeof raw === "object" && !Array.isArray(raw) && raw.contract === "algal.triage-change.v1") {
        ...
        await replay(this.service.store, reference(v.receipt), updateManifest(), { tasks: before, action: change.action }, after);
      } else {
  ...
  async export(): Promise<Transfer> {
    const history = await this.service.history(this.application), states = history.map(s => s.digest);
    if (!states.length || states.length > MAX_STATES) throw new Error("Invalid task history export");
    const records: Transfer["records"] = [], seen = new Set<Digest>(), pending = [...states]; let bytes = 0;
    while (pending.length) {
      ...
      if (value && typeof value === "object" && !Array.isArray(value) && value.contract === "algal.run.v1") kind = "receipt";
      const record = { kind, reference: ref, value }; bytes += Buffer.byteLength(canonicalize(applicationJson(record)));
  ...
  async import(input: Transfer): Promise<{ reference: Digest; application: string; head: Digest; duplicate: boolean }> {
    const transfer = parseTransfer(input), index = { contract: "algal.triage-transfer-index.v1", application: transfer.application, head: transfer.head, states: transfer.states, records: transfer.records.map(r => ({ kind: r.kind, reference: r.reference })) }, ref = hash(index), duplicate = await this.service.store.getValue(ref) !== undefined;
    if (duplicate) {
  ...
  private async transferCapture(transfer: Transfer): Promise<Capture> {
    const source = new TriageHost(this.directory, transfer.application);
  ...
  async fork(input: Transfer, newApplication: string): Promise<Capture> {
    ...
    return new TriageHost(this.directory, newApplication).initialize({ schemaVersion: captured.definition.schemaVersion, config: captured.definition.config, tasks: captured.tasks }, imported.reference);
  }
  ...
  private async computeMerge(expectedHead: Digest, source: Digest): Promise<MergeReview> {
    ...
    const localTransfer = await this.export();
    ...
    const ancestors = async (t: Transfer): Promise<Map<Digest, Task[]>> => {
      ...
      while (current) {
        ...
        const sourceHost = new TriageHost(this.directory, current.application);
...
export function parseTransfer(input: unknown): Transfer {
  const encoded = JSON.stringify(input); if (!encoded || Buffer.byteLength(encoded) > EXPORT_BYTES) throw new Error("Transfer byte bound exceeded");
...
export async function verifyTransfer(input: unknown, depth = 0): Promise<{ ok: true; states: number; receipts: number; head: Digest }> {
  if (depth > 8) throw new Error("Fork ancestry verification bound exceeded");
  const transfer = parseTransfer(input), directory = await mkdtemp(join(tmpdir(), "algal-triage-verify-"));
  try {
    const host = new TriageHost(directory, transfer.application); await copyRecords(host.service.store, transfer);
    const genesis = await getApplicationRecord(host.service.store, transfer.states[0]!, parseApplicationState), genesisTransition = await getApplicationRecord(host.service.store, genesis.transition, parseApplicationTransition);
    const origin = object(await host.service.store.getValue(genesisTransition.evidence[0]!), ["contract", "tasks", "source"]);
    if (origin.source !== null) await verifyTransfer(await host.readTransfer(reference(origin.source)), depth + 1);
    for (const head of transfer.states) {
      const state = await getApplicationRecord(host.service.store, head, parseApplicationState), transition = await getApplicationRecord(host.service.store, state.transition, parseApplicationTransition);
      ...
      const snapshot = await host.service.commit({ application: transition.application, operation: transition.operation, kind: transition.kind, expectedHead: transition.previous, revision: transition.revision, memory: transition.memory, intents: [], evidence: transition.evidence, causedBy: null });
      if (snapshot.digest !== head) throw new Error("Transfer lifecycle replay mismatch");
    }
    await host.capture();
    let receipts = 0;
    for (const r of transfer.records) if (r.kind === "receipt") { const receipt = parseRunReceipt(r.value), manifest = await host.service.store.getManifest(receipt.manifestDigest); if (!manifest || !(await verifyReceipt(r.value, manifestToJson(manifest), host.service.store)).ok) throw new Error("Transfer receipt replay failed"); receipts++; }
    return { ok: true, states: transfer.states.length, receipts, head: transfer.head };
