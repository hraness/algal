                ...
                result
                    ...
                    .map_err(|error| {
                        if error.code.len() > 64
                            ...
                            || error.wake.len() > 64
                            // cap: + a <=64-byte capability id + :sha256: + 64 hex bytes.
                            || error.wake.iter().any(|handle| handle.len() > 140)
                        {
                            ...
                            if !error.wake.is_empty() {
                                candidate["wake"] = json!(error.wake);
                            }
                            if crate::receipt::validate_effect(&candidate).is_err() {
                                let mut bounded =
