...
//! receipt is referenced by an `algal.episode-outcome.v2` record, and the dispatch
...
async fn episode(context: &DispatchContext<'_>, dir: &Path, reconciliation: bool) -> Result<Value> {
    ...
    let entry = revision
        .entrypoints
        .iter()
        .find(|entry| entry.name == binding.entrypoint)
    ...
    while let Some(program) = stack.pop() {
        if program
            ...
            .cells
            .iter()
            .any(|cell| matches!(cell["kind"].as_str(), Some("slot" | "spawn")))
        {
            journal = false;
        }
    ...
    if state.process.name != binding.process
        ...
        || state.process.max_generations != binding.max_generations
    ...
    if state.process.status == "ready" {
        state = processes
            .tick_journal(&binding.process, None, &mut host, &transports, journal, 2)
            .await?;
    } else if state.process.status == "uncertain" {
        if !reconciliation || !journal {
            return Ok(
                json!({"status":"blocked","reason":"Uncertain process requires explicit journal-safe or adapter reconciliation"}),
            );
        }
        match processes
            .recover(&binding.process, &state.digest, &mut host, &transports)
            .await
        {
            Ok(recovered) => state = recovered,
            Err(_) => {
                return Ok(
                    json!({"status":"blocked","reason":"Process journal could not establish safe recovery; adapter reconciliation required"}),
                );
            }
        }
    } else if state.process.status == "suspended"
        && reconciliation
        && state.process.generation < state.process.max_generations
    {
    ...
    if state.process.status == "suspended" {
        return Ok(
            json!({"status":"blocked","reason":if state.process.generation >= state.process.max_generations { "Episode process generation budget exhausted" } else { "Episode process is suspended awaiting an admitted wake" }}),
        );
    }
    ...
    let receipt_ref = state
        ...
        .receipt
        .as_ref()
        .ok_or_else(|| fail("Episode process has no terminal receipt"))?;
    let receipt = processes
        .store
        .get("runs", receipt_ref)?
        .ok_or_else(|| fail("Episode process receipt missing"))?;
    ...
    let outcome = put_record(
        &mut store,
        &json!({
            "contract": "algal.episode-outcome.v2",
            "binding": binding_ref,
...
mod tests {
    ...
    async fn episode_dispatch_runs_the_bound_manifest_and_settles() {
        ...
        assert_eq!(evidence["contract"], "algal.episode-outcome.v2");
        assert_eq!(evidence["binding"], outcome["result"]["binding"]);
        let receipt = store
            .get("runs", evidence["receipt"].as_str().unwrap())
            .unwrap()
            .unwrap();
        assert_eq!(receipt["manifestDigest"], manifest);
        assert_eq!(receipt["outcome"], "complete");
        let process = ProcessService::open(tmp.path())
            .unwrap()
            .inspect(outcome["result"]["process"].as_str().unwrap())
            .unwrap();
        ...
        assert_eq!(
            reconcile_episode(&context, tmp.path()).await.unwrap(),
            outcome
        );
        assert_eq!(
            dispatch_episode(&context, tmp.path()).await.unwrap(),
            outcome
        );
    }
    ...
    async fn existing_process_reuse_requires_exact_arguments_and_generation_budget() {
        for mismatch in [None, Some("arguments"), Some("budget")] {
            ...
            let dispatch =
                episode_dispatch(&manifest, &revision, &arguments, &hashed(&intent.value));
            let snapshot = snapshot();
            let context = DispatchContext {
                current: &snapshot,
                snapshot: &snapshot,
            ...
            let name = dispatch.plan.value()["binding"]["process"]
                .as_str()
                .unwrap()
                .to_owned();
            ...
            if mismatch == Some("arguments") {
                args["src"]["value"] = json!("different");
            }
            let ready = processes
                .create(
                    &name,
                    program,
                    args,
                    if mismatch == Some("budget") { 5 } else { 4 },
                    &Host::default(),
                    &Transports::new(),
                )
                .unwrap();
    ...
    async fn explicit_recovery_resumes_pure_intent_but_never_unknown_write() {
        for unknown_write in [false, true] {
            ...
            pending.status = "uncertain".into();
            pending.previous = Some(ready.digest);
            pending.cause = Some("start".into());
            let pending_ref = processes
                .store
                .put("values", &serde_json::to_value(pending).unwrap())
                .unwrap();
            let mut journal =
                crate::journal::Journal::create(tmp.path(), &name, &pending_ref, &manifest, 2)
                    .unwrap();
            if unknown_write {
                journal
                    .before(crate::journal::Binding {
                        request_digest: hashed(&json!("request")),
                        executor: "fixture-write".into(),
                        configuration_digest: hashed(&json!("config")),
                        idempotency_key: hashed(&json!("effect")),
                        recovery: "never".into(),
                    })
                    .unwrap();
            }
            crate::lease::write(
                &tmp.path().join("processes").join(&name).join("head.json"),
                &json!({"contract":"algal.process-head.v1","name":name,"record":pending_ref}),
                true,
            )
            .unwrap();
            assert_eq!(
                dispatch_episode(&context, tmp.path()).await.unwrap()["status"],
                "blocked"
            );
            ...
            assert_eq!(
                recovered["status"],
                if unknown_write { "blocked" } else { "settled" }
            );
            ...
            if unknown_write {
                assert_eq!(state.digest, pending_ref);
            }
        }
    }

    #[tokio::test]
    async fn large_receipt_settles_via_bounded_outcome_reference() {
        let tmp = tempdir().unwrap();
        let (_, old_revision, _) = seed(tmp.path());
        let mut store = Store::open(tmp.path(), true).unwrap();
        let manifest = Manifest::parse(&json!({"contract":"algal.organism.v1","key":"organism:large-episode","name":"Large episode",
            "cells":(0..6).map(|i| json!({"id":format!("value{i}"),"kind":"const","outputs":{"value":{"type":"text","value":"x".repeat(60_000)}}})).collect::<Vec<_>>(),"edges":[]})).unwrap();
        let manifest_ref = store.admit(&manifest).unwrap();
        let mut revision = get_record(&store, &old_revision).unwrap();
        revision["entrypoints"][0]["manifest"] = json!(manifest_ref);
        let revision = put_record(&mut store, &revision).unwrap();
        let arguments = put_record(&mut store, &json!({})).unwrap();
        let intent = parse_intent(&json!({"contract":"algal.application-intent.v1","application":"parity",
            "operation":hashed(&json!("large")),"ordinal":0,"kind":"start-episode","entrypoint":"run","input":arguments})).unwrap();
        let dispatch =
            episode_dispatch(&manifest_ref, &revision, &arguments, &hashed(&intent.value));
        let snapshot = snapshot();
        let context = DispatchContext {
            current: &snapshot,
            snapshot: &snapshot,
        ...
        let result = dispatch_episode(&context, tmp.path()).await.unwrap();
        assert_eq!(result["status"], "settled");
        let outcome = get_record(&store, result["result"]["outcome"].as_str().unwrap()).unwrap();
        assert!(crate::canonical::canonical(&outcome).unwrap().len() < 1024);
        let receipt = store
            .get("runs", outcome["receipt"].as_str().unwrap())
            .unwrap()
            .unwrap();
        assert!(crate::canonical::canonical(&receipt).unwrap().len() > 262_144);
        assert_eq!(
            reconcile_episode(&context, tmp.path()).await.unwrap(),
            result
        );
