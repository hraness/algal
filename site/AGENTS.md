...
- `chrome.ts`, `pages/`, `styles.css`, and `build.ts` own the static ALGAL website and its executable example projections. `pages/` holds the home, tour, use-cases, and living fragments composed into `chrome.ts`'s shared shell.
...
- Preserve executable examples, navigation, the `{{PLACEHOLDER}}` tokens in `pages/`, and the Paper palette and light/dark/system appearance. A design change does not rewrite product copy as a side effect.
