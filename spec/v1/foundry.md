...
A habitat schedule runs several foundry and search configs against one habitat budget account. Both CLIs run one with `algal foundry schedule <schedule.json>` and verify its record with `algal foundry schedule-verify <record.json>`.
...
Scheduling application experiment evaluations and orders other than `round-robin` are proposed.
